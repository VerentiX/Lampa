...
void main() {
  ...
  group('Lampa: GitHub checker disabled', () {
    test('enabled is false for consumer build', () {
      expect(UpdateChecker.enabled, isFalse);
    });

    test('forceCheck skips without network', () async {
      final r = await UpdateChecker.I.forceCheck(localVersion: '1.5.1');
      expect(r.kind, UpdateCheckKind.skipped);
    });

    test('hydrate clears stale latest', () async {
      UpdateChecker.I.latest.value = const UpdateInfo(
        tag: 'v2.21.0',
        name: 'stale',
        htmlUrl: 'https://example.com',
      );
      await UpdateChecker.I.hydrate(localVersion: '1.5.1');
