...
void main() {
  group('expandPreset (spec §033)', () {
    ...
    test(
      'required var без explicit + defaultValue → substituted by default',
      () {
        final preset = _ruDirect();
        final rule = CustomRulePreset(
          name: 'Russian domains direct',
    ...
    test(
      'required var + no value + empty default → empty fragments + warn',
      () {
        ...
        final rule = CustomRulePreset(name: 'X', presetId: 'broken');
    ...
    test('remote rule_set + НЕТ cached path → rule_set skipped + warning + '
      ...
      expect(
        ...
        isEmpty,
        reason:
            'rule без своего rule_set → drop, иначе sing-box падает '
    ...
    test(
      '§045: geoip on + .srs cached → оба rule_set + array routing rule',
      () {
    ...
    test('§045: geoip on + .srs НЕ cached → даунгрейд до single ru-domains '
      ...
      expect(
        f.warnings.any(
          (w) => w.contains('geoip-ru') && w.contains('no cached'),
        ),
        isTrue,
      );
    });
    ...
    test('§045: enabled bool literal true → фрагмент включается', () {
      ...
      expect(f.ruleSets.length, 1);
      expect(
        f.ruleSets.first.containsKey('enabled'),
        isFalse,
    ...
    test(
      '§045: rule.rule_set массив, оба tag\'а missing → rule dropped + warning',
      () {
        final preset = SelectableRule(
          label: 'X',
          presetId: 'x',
        ...
        expect(
          f.warnings.any(
            (w) => w.contains('none of') && w.contains('a') && w.contains('b'),
          ),
          isTrue,
        );
      },
    ...
    test(
      'outbound override == "reject" → action:reject, outbound удаляется',
      () {
        final preset = _ruDirect();
        final rule = CustomRulePreset(
          name: 'Block Russian',
    ...
    test('§162 default_value=="reject" в @outbound (пикер не трогали) → '
      ...
      final f = expandPreset(rule, preset);
      expect(f.routingRules.single, {
        'rule_set': 'unknown-apps',
        'action': 'reject',
      });
      expect(
        f.routingRules.single.containsKey('outbound'),
        isFalse,
    ...
    test(
      'deep-copy isolation: source preset не мутируется после expansion',
      () {
        final preset = _ruDirect();
        final originalRuleSet = Map<String, dynamic>.from(
          preset.ruleSets.first,
        );
    ...
    test('routing rule без outbound/action после substitute → dropped', () {
      ...
      final rule = CustomRulePreset(name: 'X', presetId: 'weird');
  ...
  group('mergeFragments (spec §033)', () {
    ...
    test('порядок детерминирован — bundle A затем B', () {
      final a = PresetFragments(
        dnsServers: [
          {'tag': 'a', 'type': 'udp'},
        ],
      );
      final b = PresetFragments(
        dnsServers: [
          {'tag': 'b', 'type': 'udp'},
        ],
      );
  ...
  group('expandPreset — FakeIP (§228)', () {
    ...
    test('РЕАЛЬНЫЙ fakeip: dns_servers + dns_rules эмитятся (rule_enable не '
      ...
      final fakeip = tpl.selectableRules.firstWhere(
        (r) => r.presetId == 'fakeip',
      );
      ...
      expect(
        f.warnings,
        isEmpty,
        reason: 'rule_enable не должна ронять пресет',
      );
    ...
    test(
      'ru-direct.hasOutboundAffordance == true (есть var:outbound и rule)',
      () {
  ...
  group('expandPreset — rule массив (§246)', () {
    ...
    test(
      'override vpn-1 → #if-гейт роняет resolve; route получает override',
      () {
        final f = expandPreset(
          CustomRulePreset(
            name: 'X',
        ...
        expect(f.warnings, isEmpty);
        expect(
          f.routingRules.single,
          {'rule_set': 'ru-domains', 'outbound': 'vpn-1'},
          reason:
              'resolve ipv4_only вреден в туннеле (IPv6 там живёт) — '
    ...
    test(
      'dangling rule_set в одном элементе → дропается только он + warning',
      () {
        final preset = SelectableRule(
          label: 'X',
          presetId: 'x',
    ...
    test(
      'force_ipv4=false → resolve-элемент выпадает, остаётся только route',
      () {
        final f = expandPreset(
          CustomRulePreset(
            name: 'X',
    ...
    test('SelectableRule.fromJson: канонический ключ "rules" (массив) — '
      ...
      expect(
        ...
        2,
        reason:
            'ключ "rules" должен читаться (регрессия: пресет молча '
  ...
  group('§246 e2e — реальный wizard_template.json', () {
    ...
    test('traffic-processing rejects only QUIC UDP/443 by default', () {
      final preset = realPreset('traffic-processing');
      final enabled = expandPreset(
        CustomRulePreset(name: 'Traffic', presetId: 'traffic-processing'),
        preset,
      );

      final rejects = enabled.routingRules
          .where((rule) => rule['action'] == 'reject')
          .toList();
    ...
    test('traffic-processing switches Roscom routing at p5 boundary', () {
      ...
      final p5 = expandPreset(
        CustomRulePreset(
          name: 'Traffic',
          presetId: 'traffic-processing',
          varsValues: const {
            'priority_p5_plus_active': 'true',
            'priority_proxy_outbound': 'vpn-3',
          },
        ),
        preset,
        srsPaths: cached,
    ...
    test(
      'vowifi (дефолты: ike_ports=true) → route по домену + route по портам',
      () {
    ...
    test('ru-direct несёт mcc250-суффикс ePDG (РФ-операторы)', () {
      ...
      final services = preset.ruleSets.firstWhere(
        (rs) => rs['tag'] == 'ru-services',
      );
      final suffixes =
    ...
    test('ru-direct (дефолты: force_ipv4=true) → пара resolve+route; '
      ...
      expect(
        ...
        2,
        reason:
            'resolve + route; 0 или 1 = движок потерял правило '
      ...
      expect(
        ...
        'dns_ru',
        reason:
            'server из default dns_server — §354 группа (битую ссылку '
      ...
      expect(
        ...
        2,
        reason:
            'AAAA-гейт + маршрут; 1 = #if-гейт потерян '
      ...
      expect(
        dnsRoute.containsKey('ip_version'),
        isFalse,
        reason:
            'маршрут безусловный — не-A/AAAA-запросы (HTTPS type 65) '
            'тоже должны идти в @dns_server',
      );
      expect(
        dnsRoute.containsKey('strategy'),
        isFalse,
    ...
    test('ru-direct (дефолты) → dns_ru + три члена по разным путям', () {
      ...
      expect(byTag('yandex_dot')['detour'], 'vpn-1');
      expect(
        byTag('yandex_doh').containsKey('detour'),
        isFalse,
    ...
    test(
      'ru-direct: force_ipv4=false → route без resolve, DNS без AAAA-гейта',
      () {
        final preset = realPreset('ru-direct');
        final f = expandPreset(
          CustomRulePreset(
    ...
    test('ru-inside (srs закэширован) → пара resolve+route', () {
      ...
      expect(f.routingRules[0]['strategy'], 'ipv4_only');
      expect(
        f.routingRules[0].containsKey('server'),
        isFalse,
    ...
    test('fcm-push (дефолты) → and из одной ветви, без package_name', () {
      ...
      expect(
        r.containsKey('package_name'),
        isFalse,
        reason:
            'gms_only=false → ветвь выпала целиком (Dropped), '
    ...
    test('fcm-push: gms_only=true → package_name отдельной ветвью and, '
      ...
      expect(
        ...
        isFalse,
        reason:
            'на верхнем уровне logical это unknown field → ядро '
  ...
  group('expandPreset — dns_rules array (§253)', () {
    ...
    test(
      'оба ключа сразу → dns_rules побеждает dns_rule (контракт TEMPLATE.md)',
      () {
