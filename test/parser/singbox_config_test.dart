...
import 'package:lxbox/models/channel.dart' show UrltestMode;
...
void main() {
  group('§368 формы входа', () {
    ...
    test('flavor распознаётся для каждой формы', () {
      expect(
        (decode(jsonEncode(ob)) as JsonConfig).flavor,
      ...
      expect(
        (decode(jsonEncode(cfg([ob]))) as JsonConfig).flavor,
  ...
  group('§368 P2/§342 — два прохода', () {
    ...
    test('порядок списка — авторский, не отсортированный', () {
      ...
      expect(r.map((n) => n.server).toList(), ['p1.com', 'p2.com', 's.com']);
    });
  ...
  group('§368 P5 — ничего не теряется молча', () {
    test('неизвестный type → warning на соседе', () {
      ...
      expect(
        r.single.warnings,
        contains(const UnsupportedProtocolWarning('shadowtls')),
      );
    });
    ...
    test('битая форма outbound не роняет соседей', () {
      ...
      expect(r.single.server, 'a.com');
      expect(
        r.single.warnings.whereType<UnsupportedProtocolWarning>(),
        isNotEmpty,
      );
    });
  ...
  group('§368 §5 — группы', () {
    Map<String, dynamic> group(
      ...
      String type = 'urltest',
      Map<String, dynamic>? extra,
    }) => {'type': type, 'tag': 'auto', 'outbounds': members, ...?extra};
    ...
    test('параметры маппятся', () {
        ...
        cfg([
          ...
          group(
            ...
            extra: {
              ...
              'interval': '5m',
              'active_check_interval': '30s',
              'active_check_failures': 2,
              'tolerance': 100,
              'idle_timeout': '10m',
      ...
      expect(g.params.activeCheckInterval, '30s');
      expect(g.params.activeCheckFailures, 2);
      expect(g.params.tolerance, 100);
      expect(g.params.idleTimeout, '10m');
      expect(g.params.interruptExistConnections, isTrue);
    });

    test('priority round_robin сохраняется из подписки', () {
      final r = parse([
        cfg([
          vless('a', 'a.com'),
          group(
      ...
      final g = r.last as AutoSelectSpec;
      expect(g.params.mode, UrltestMode.roundRobin);
      expect(g.params.pool, 1);
      expect(g.params.poolTolerance, 0);
      expect(g.params.stickyHash, isEmpty);
      expect(g.params.priorityRecheckInterval, '10m');
      expect(
        (g.emitRaw(const TemplateVars()).map['balancer']
            as Map)['priority_recheck_interval'],
        '10m',
      );
    });
    ...
    test('отсутствующие параметры → наши дефолты', () {
      ...
      expect(g.params.activeCheckInterval, d.activeCheckInterval);
      expect(g.params.activeCheckFailures, d.activeCheckFailures);
      expect(g.params.tolerance, d.tolerance);
      expect(g.params.mode, d.mode);
    });

    test('старый urltest мигрирует с частого full-pool на active health', () {
      final r = parse([
        cfg([
          vless('a', 'a.com'),
          group(
            ['a'],
            extra: {'interval': '30s', 'interrupt_exist_connections': false},
          ),
        ]),
      ]);
      const d = AutoSelectParams();
