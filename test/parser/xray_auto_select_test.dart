...
void main() {
  ...
  group('схема имён элемента (§322 §6.5)', () {
    ...
    test('пустой тег → индекс, без хвостового пробела', () {
      final labels = labelsOf([
        plain('Пул', [vless('1.1.1.1', tag: ''), vless('2.2.2.2', tag: 'b')]),
      ]);
  ...
  group('маппинг стратегии', () {
    ...
    test('roundRobin → round_robin по всему набору', () {
      final a = parse([
        withBalancer(
          'A',
          [vless('1.1.1.1', tag: 'proxy-1'), vless('2.2.2.2', tag: 'proxy-2')],
          type: 'roundRobin',
          settings: const {},
        ),
      ]).whereType<AutoSelectSpec>().single;
    ...
    test('leastLoad с expected>1 → round_robin', () {
      expect(
        build(type: 'leastLoad', settings: {'expected': 7}).params.mode,
    ...
    test('expected≤1 → least_test, а не пул из одного', () {
      ...
      for (final e in [1, 0]) {
        expect(
          build(type: 'leastLoad', settings: {'expected': e}).params.mode,
    ...
    test('pool_tolerance клампится по 15000 (SPEC 019)', () {
      expect(
        build(settings: {'maxRTT': '30s'}).params.poolTolerance,
        kMaxPoolTolerance,
      );
    });
  ...
  group('битые формы balancers не роняют парсинг', () {
    ...
    test('balancers пустой / не массив / мусор внутри', () {
      ...
      survives('объект вместо массива', {
        'balancers': {'tag': 'B'},
      });
      survives('строка внутри массива', {
        'balancers': const ['мусор'],
      });
    });
  ...
  group('формат эмиссии urltest (ядро SPEC 019)', () {
    ...
    test('round_robin: pool/tolerance/sticky ВНУТРИ balancer{}', () {
      ...
      expect(
        m.containsKey('pool'),
        isFalse,
        reason: 'плоский pool = unknown field',
      );
    ...
    test('least_test: ни mode, ни balancer (бит-в-бит апстрим)', () {
      ...
      expect(m['active_check_interval'], '30s');
      expect(m['active_check_failures'], 2);
    });

    test('interrupt_exist_connections эмитится всегда', () {
      expect(
        emit(const AutoSelectParams())['interrupt_exist_connections'],
        isFalse,
      );
      expect(
        emit(
          const AutoSelectParams(interruptExistConnections: true),
    ...
    test('снятая липкость → sentinel ["none"] (§210)', () {
      ...
      final m = emit(
        const AutoSelectParams(mode: UrltestMode.roundRobin, stickyHash: []),
      );
      expect((m['balancer'] as Map)['sticky_hash'], ['none']);
    });

    test('priority_recheck_interval эмитится внутри balancer', () {
      final m = emit(
        const AutoSelectParams(
          mode: UrltestMode.roundRobin,
          pool: 1,
          priorityRecheckInterval: '10m',
        ),
      );
      expect((m['balancer'] as Map)['priority_recheck_interval'], '10m');
    });
  });

  group('URI-форма autogroup:// (§7)', () {
    AutoSelectSpec rt(AutoSelectSpec a) =>
        parseUri(a.toUri()) as AutoSelectSpec;
    ...
    test('правило переживает round-trip', () {
      ...
      expect(b.params.priorityRecheckInterval, '10m');
    });
