...
void main() {
  group('parseSingboxEntry', () {
    test(
      '§115: raw sing-box JSON flow=vision + transport → emit гасит flow',
      () {
    ...
    test('vless outbound fixture', () {
      final j =
          jsonDecode(
                File(
                  'test/fixtures/json/singbox_vless_outbound.json',
                ).readAsStringSync(),
              )
    ...
    test('wireguard endpoint fixture', () {
      final j =
          jsonDecode(
                File(
                  'test/fixtures/json/singbox_wg_endpoint.json',
                ).readAsStringSync(),
              )
              as Map<String, dynamic>;
      final spec = parseSingboxEntry(j);
  ...
  group('parseXrayOutbound', () {
    test('reality array fixture', () {
      final j =
          jsonDecode(
                File(
                  'test/fixtures/json/xray_array_reality.json',
                ).readAsStringSync(),
              )
              as List;
    ...
    test('§335: Xray без encryption → поля в конфиге нет', () {
      ...
      expect(
        spec.emit(TemplateVars.empty).map.containsKey('encryption'),
        isFalse,
      );
    });
    ...
    test('§169: Xray reality + битый publicKey → plain TLS, без reality', () {
      ...
      expect(
        spec.tls.reality,
        isNull,
        reason: 'мусорный publicKey → нет reality',
      );
    });
  });
  ...
  group('§310 parseXrayElement — все ноды элемента', () {
    ...
    test('3 равноправных VLESS → 3 ноды, имена различимы', () {
      ...
      expect(
        nodes.map((n) => n.label).toSet().length,
        3,
    ...
    test(
      '§018 регрессия: dialerProxy → 1 нода с detour, цель НЕ отдельной нодой',
      () {
    ...
    test(
      '§335+§321: dialerProxy не теряет encryption (регрессия _withChain)',
      () {
        const enc = 'mlkem768x25519plus.native.0rtt.AbCd-EfGh_IjKl0123456789';
        final main = vless('proxy', 'main.example');
        (main['streamSettings'] as Map)['sockopt'] = {'dialerProxy': 'jump'};
    ...
    test('remarks пустой → метка из тега outbound', () {
      final nodes = parseXrayElement({
        'outbounds': [
          vless('proxy', 'a.example'),
          vless('backup', 'b.example'),
        ],
      });
      expect(nodes.length, 2);
  ...
  group('идентичность parser ↔ builder', () {
    ...
    test(
      'hysteria (форма форка) → ключ hysteria2|…, порт как у конвертера',
      () {
        final nodes = parseXrayElement({
          'remarks': 'HY',
          'outbounds': [
  ...
  group('§169 _tlsFromSingbox pbk validation', () {
    test('sing-box reality + битый public_key → plain TLS, без reality', () {
      ...
      expect(
        spec.tls.reality,
        isNull,
        reason: 'битый public_key → нет reality',
      );
      expect(spec.tls.serverName, 'w.example');
    });
  });

  group('XHTTP extended JSON fields', () {
    test('Xray xhttpSettings aliases produce the complete core transport', () {
      final nodes = parseXrayElement({
        'remarks': 'xhttp',
        'outbounds': [
          {
      ...
      final transport =
          (nodes.whereType<VlessSpec>().single.transport as XhttpTransport);
      final emitted = transport.toSingbox(TemplateVars.empty).$1;
      expect(emitted['session_placement'], 'header');
      expect(emitted['session_key'], 'X-Session');
      expect(emitted['seq_placement'], 'header');
      expect(emitted['seq_key'], 'X-Seq');
      expect(emitted['x_padding_obfs_mode'], isTrue);
      expect(emitted['x_padding_header'], 'X-Request-Meta');
      expect(emitted['x_padding_key'], 'meta');
      expect(emitted['x_padding_method'], 'tokenish');
      expect(emitted['x_padding_placement'], 'header');
      expect(emitted.containsKey('uplink_chunk_size'), isFalse);
    });

    test('sing-box snake_case session and extended fields survive import', () {
      final spec =
          parseSingboxEntry({
                'type': 'vless',
                'tag': 'xhttp',
                ...
                'transport': {
                  'type': 'xhttp',
                  'session_id_length': '8-12',
                  'session_id_table': 'abcXYZ019',
                  'session_placement': 'header',
                  'session_key': 'X-Session',
                  'seq_placement': 'header',
                  'seq_key': 'X-Seq',
                  'x_padding_obfs_mode': true,
                  'x_padding_header': 'X-Request-Meta',
                },
              })
              as VlessSpec;
      final emitted = spec.transport!.toSingbox(TemplateVars.empty).$1;
      expect(emitted['sessionIDLength'], '8-12');
      expect(emitted['sessionIDTable'], 'abcXYZ019');
      expect(emitted['session_placement'], 'header');
      expect(emitted['seq_placement'], 'header');
      expect(emitted['x_padding_obfs_mode'], isTrue);
    });
