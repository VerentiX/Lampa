...
void main() {
  group('XHTTP', () {
    ...
    test('минимальный xhttp (только type) → дефолты, без лишних ключей', () {
      final (m, _) = (parseTransport({'type': 'xhttp'}) as XhttpTransport)
          .toSingbox(TemplateVars.empty);
      expect(m['type'], 'xhttp');
  ...
  group('XHTTP §127 full params', () {
    test('golden: все поля camelCase → core transport (§8.1)', () {
      final t =
          parseTransport({
                'type': 'xhttp',
    ...
    test('session ID fields from extra are parsed and emitted verbatim', () {
      final t =
          parseTransport({
                'type': 'xhttp',
                'extra': '{"sessionIDLength":10,"sessionIDTable":"abcXYZ019"}',
              })
              as XhttpTransport;
      expect(t.sessionIdLength, '10');
      expect(t.sessionIdTable, 'abcXYZ019');
      final m = t.toSingbox(TemplateVars.empty).$1;
      expect(m['sessionIDLength'], '10');
      expect(m['sessionIDTable'], 'abcXYZ019');
    });
    ...
    test(
      '§217 GET+не-packet-up → сброс+warning; GET+packet-up → сохраняется',
      () {
        ...
        final t4 = parseTransport({
          'type': 'xhttp',
          'session_placement': 'bogus',
        })!;
    ...
    test('path с ?-хвостом обрезается', () {
      final t =
          parseTransport({'type': 'xhttp', 'path': '/GaMeOpTiMiZeR?ed=2048'})
              as XhttpTransport;
    ...
    test('round-trip: parseTransport(transportToQuery(golden)) ≈ golden', () {
      ...
      expect(
        back.toSingbox(TemplateVars.empty).$1,
