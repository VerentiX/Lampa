...
void main() {
    ...
    test('full channel with auto → JSON → Channel', () {
      const original = Channel(
        ...
        auto: ChannelAuto(
          ...
          interval: '3m',
          activeCheckInterval: '45s',
          activeCheckFailures: 3,
          tolerance: 30,
          idleTimeout: '20m',
      ...
      expect(restored.auto!.activeCheckInterval, '45s');
      expect(restored.auto!.activeCheckFailures, 3);
  ...
  group('§208 — ChannelAuto balancer (round_robin)', () {
    test(
      'дефолты: leastTest, pool 3, poolTolerance 0, sticky process+domain',
      () {
        const a = ChannelAuto();
    ...
    test(
      'старый JSON без mode/balancer → дефолты (обратная совместимость)',
      () {
