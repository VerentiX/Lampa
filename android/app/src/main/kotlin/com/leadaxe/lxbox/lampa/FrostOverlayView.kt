...
class FrostOverlayView @JvmOverloads constructor(
    ...
    /** Circular cutout so ice stays under the power button (Lampa FAB stacking). */
    private var buttonHoleR = 0f
    ...
    /** Half-res bake — ~4× fewer pixels; FILTER upscale still looks soft. */
    private val bakeScale = 0.45f
    /** After this progress, live crack draw switches to a baked blit (cheap). */
    private val bakeAtProgress = 0.78f
    ...
    fun freezeFrom(source: View, clipTo: View? = null) {
        // Temporarily disabled — cracks not drawn.
    ...
    private fun updateBounds(source: View, clipTo: View?) {
        ...
        // Keep ice under the FAB face (same stacking as Lampa elevation).
        buttonHoleR = max(source.width, source.height) / 2f + dp(6f)
    ...
    private fun animateTo(target: Float, freeze: Boolean) {
        ...
        animator = ValueAnimator.ofFloat(start, target).apply {
            ...
            addUpdateListener {
                ...
                // Early bake: finish growing cracks once, then idle blit only.
            ...
            addListener(object : android.animation.AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: android.animation.Animator) {
                    if (target <= 0.01f) {
                        ...
                        if (snapshot == null) captureSnapshot()
                        else invalidate()
                    }
                }
            })
    ...
    private fun rebuildPattern() {
        ...
        android.util.Log.d(
            "LampaFrost",
            "ready cracks=${cracks.size} junctions=${junctions.size} sparkles=${sparkles.size}"
        )
    }
    ...
    private fun buildCracks(rnd: Random): Pair<List<Crack>, List<Junction>> {
        val result = ArrayList<Crack>(80)
        val nodes = ArrayList<Junction>(32)

        val burst = 12
        for (i in 0 until burst) {
            val a = i * (Math.PI.toFloat() * 2f / burst) +
                (rnd.nextFloat() - 0.5f) * 0.55f
            val len = dp(18f) + rnd.nextFloat() * dp(26f)
            val pts = buildIceFissure(originX, originY, a, len, 6 + rnd.nextInt(4), rnd)
            if (pts.size >= 2) {
                result += Crack(
                    points = pts,
                    width = dp(1.5f) + rnd.nextFloat() * dp(1.0f),
                    delay = i * 0.006f,
                    branch = false,
                    taper = 0.18f
                )
        ...
        for (i in 0 until mainCount) {
            ...
            val forks = 1 + rnd.nextInt(2)
            repeat(forks) { fork ->
                val fromIndex = ((0.25f + fork * 0.22f + rnd.nextFloat() * 0.1f) * (points.size - 1))
                    .toInt()
                ...
                nodes += Junction(from.x, from.y, branchDelay, branchWidth * 0.6f)
            }
        }
        ...
        for (i in 0 until 16) {
            ...
            val startD = maxLen * (0.12f + rnd.nextFloat() * 0.38f)
            val len = maxLen * (0.22f + rnd.nextFloat() * 0.35f)
            val start = CrackPoint(originX + cos(angle) * startD, originY + sin(angle) * startD)
            val pts = buildIceFissure(start.x, start.y, angle, len, 7 + rnd.nextInt(5), rnd)
            if (pts.size >= 2) {
                result += Crack(
                    points = pts,
                    width = dp(0.45f) + rnd.nextFloat() * dp(0.55f),
                    delay = 0.18f + i * 0.015f,
                    branch = true,
                    taper = 0.35f
                )
            }
        ...
        android.util.Log.d(
            "LampaFrost",
            "pattern cracks=${result.size} junctions=${nodes.size}"
        )
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val clipped = clipOutButton(canvas)
        try {
            val baked = snapshot
            if (baked != null && !baked.isRecycled && progress >= bakeAtProgress) {
    ...
    /** Returns true if canvas.save() was called (caller must restore). */
    private fun clipOutButton(canvas: Canvas): Boolean {
        if (buttonHoleR <= 0f) return false
        holePath.rewind()
        holePath.fillType = Path.FillType.EVEN_ODD
        holePath.addRect(0f, 0f, width.toFloat(), height.toFloat(), Path.Direction.CW)
        holePath.addCircle(originX, originY, buttonHoleR, Path.Direction.CW)
        canvas.save()
    ...
    private fun drawFrostScene(canvas: Canvas, t: Float) {
        ...
        if (ms > 8f) {
            android.util.Log.w("LampaFrost", "slow drawFrostScene ${"%.1f".format(ms)}ms cracks=${cracks.size}")
        }
    }
    ...
    private fun drawCrack(canvas: Canvas, crack: Crack, spread: Float) {
        ...
        crackGlowPaint.strokeWidth = deepW * (if (crack.branch) 2.2f else 2.8f)
        crackGlowPaint.alpha = (grow * spread * tipBoost * 50 * alphaScale).toInt().coerceIn(0, 72)
        canvas.drawPath(tmpPath, crackGlowPaint)

        // Dark crack body.
        crackDeepPaint.color = 0xFF071420.toInt()
        crackDeepPaint.strokeWidth = deepW
        crackDeepPaint.alpha = (grow * spread * 200 * alphaScale).toInt().coerceIn(0, 220)
        canvas.drawPath(tmpPath, crackDeepPaint)

        // Bright frost slit.
        crackMidPaint.color = 0xFFF2FAFF.toInt()
        crackMidPaint.strokeWidth = deepW * 0.4f
        crackMidPaint.alpha = (grow * spread * 245 * alphaScale).toInt().coerceIn(0, 255)
        canvas.drawPath(tmpPath, crackMidPaint)

        if (grow in 0.04f..0.96f) {
            val tipA = ((1f - abs(grow - 0.55f) * 1.4f).coerceIn(0.2f, 1f) * spread * 220).toInt()
                .coerceIn(0, 230)
            sparkPaint.color = 0xFFFFFFFF.toInt()
            sparkPaint.alpha = tipA
            canvas.drawCircle(endX, endY, deepW * 0.35f + dp(0.35f), sparkPaint)
        }
    }
