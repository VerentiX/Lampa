...
import com.leadaxe.lxbox.lampa.LampaCeremony
import com.leadaxe.lxbox.lampa.LampaPowerViewFactory
...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        flutterEngine.platformViewsController.registry.registerViewFactory(
            "com.leadaxe.lxbox/lampa_power",
            LampaPowerViewFactory(flutterEngine.dartExecutor.binaryMessenger),
        )

        MethodChannel(
            flutterEngine.dartExecutor.binaryMessenger,
            "com.leadaxe.lxbox/lampa_ceremony",
            ...
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "legacyLampaSubscriptions" -> {
                        result.success(readLegacyLampaSubscriptions())
                    }
                    "openUrl" -> {
        ...
        MethodChannel(
            flutterEngine.dartExecutor.binaryMessenger,
            "com.leadaxe.lxbox/lampa_update",
        ).setMethodCallHandler { call, result ->
            when (call.method) {
                "installApk" -> {
    ...
    private fun readLegacyLampaSubscriptions(): List<Map<String, String>> {
        return runCatching {
            MMKV.initialize(applicationContext)
            val main = MMKV.mmkvWithID("MAIN", MMKV.MULTI_PROCESS_MODE)
            val subscriptions = MMKV.mmkvWithID("SUB", MMKV.MULTI_PROCESS_MODE)
            val ids = JSONArray(main.decodeString("SUB_IDS") ?: "[]")
            buildList {
                for (index in 0 until ids.length()) {
                    val raw = subscriptions.decodeString(ids.optString(index)) ?: continue
                    val item = JSONObject(raw)
                    val url = item.optString("url").trim()
                    if (url.isNotEmpty()) {
                        add(mapOf(
                            "url" to url,
                            "name" to item.optString("remarks", "Хаттабыч VPN"),
                        ))
                    }
                }
            }
        }.getOrElse {
            Log.w(TAG, "Legacy Lampa MMKV migration failed", it)
            emptyList()
        }
    }
    ...
    private fun installApkFile(path: String): Boolean {
        return try {
            val file = File(path)
            if (!file.isFile) {
                Log.w(TAG, "installApkFile: missing $path")
                return false
            }
            ...
            Log.w(TAG, "installApkFile failed", e)
            false
        }
    }
