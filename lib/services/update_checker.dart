...
class UpdateChecker {
  ...
  /// When false, hydrate / maybeCheck / forceCheck are no-ops and UI should
  /// hide GitHub update chrome (settings, about, first-run prompt).
  ...
  Future<void> maybeCheck({required String localVersion}) async {
    ...
    if (_inFlight) return;
    if (_isDevBuild(localVersion)) return;
    final auto = await SettingsStorage.getAutoCheckUpdates();
    if (!auto) return;
