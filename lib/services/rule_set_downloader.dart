...
class RuleSetDownloader {
  ...
  static Future<DownloadResult> fetch(
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      if (onProgress != null && client == null) {
        final streamed = await _fetchResumable(
          id: id,
          url: url,
  ...
  /// Range resume into `.part`, then rename. Null = retry the attempt.
  static Future<DownloadResult?> _fetchResumable({
    required String id,
    required String url,
    required bool conditional,
    required RuleSetMeta prev,
    ...
    try {
      ...
      req.headers.set(HttpHeaders.userAgentHeader, 'Lampa-Mobile');
      final etag = prev.etag;
      if (conditional &&
          existing == 0 &&
          etag != null &&
      ...
      if (resp.statusCode >= 400 && resp.statusCode < 500) {
        return finish(DownloadOutcome.failed, error: 'HTTP ${resp.statusCode}');
      }
