...
Future<FetchResult> _fetch(SubscriptionSource source, http.Client client) async {
  switch (source) {
    case UrlSource(
      ...
      // Managed Хаттабыч URLs: try primary → fallback → reserve hosts
      // (old Lampa behaviour). Each host still gets transient retries.
      final candidates = SubscriptionUrlResolver.candidateUrls(u);
      Object? lastErr;
        ...
        for (var attempt = 0; attempt < 3; attempt++) {
          try {
            final resp = await client
                .get(Uri.parse(candidate), headers: reqHeaders)
                .timeout(t);
            if (resp.statusCode >= 400 && resp.statusCode < 500) {
              throw HttpException('HTTP ${resp.statusCode} for $candidate');
            }
            ...
            if (candidate != candidates.first) {
              AppLog.I.info(
                'Subscription fetched via fallback host: $candidate',
              );
            }
            ...
            lastErr = e;
            // 4xx on this host — try next host (mirror may still work).
            if (e.message.contains(RegExp(r'HTTP 4\d\d'))) break;
