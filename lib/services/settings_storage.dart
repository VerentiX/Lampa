...
class SettingsStorage {
  ...
  static const _appFeatureFlagVars = <String>{
    ...
    'lampa_device_id',
    'lampa_connection_remark',
    'lampa_sub_update_hours',
    'lampa_app_check_hours',
    'lampa_geo_check_hours',
  ...
  static Future<void> setVar(String name, String value, {bool flush = true}) =>
  ...
  static Future<void> saveEnabledGroups(
    Set<String> groups, {
    bool flush = true,
  ...
  @visibleForTesting
  static Future<Channel> addChannel({String? label}) =>
      _addChannel(label: label);
  ...
  static Future<void> saveCustomRules(
  ...
  static Future<void> saveIdleSuspendReachable(
    String threshold, {
    bool flush = true,
  ...
  static Future<void> saveDnsServers(
  ...
  static Future<void> saveDnsRules(String rulesJson) =>
      _saveDnsRules(rulesJson);
  ...
  /// пишет, что дали.
  static Future<void> saveDnsRulesList(
  ...
  static Future<List<Map<String, String>>> getWifiHistory() =>
      _getWifiHistory();
  ...
  // Consumer (Lampa) auto-update intervals. 0 = manual only.
  static const lampaIntervalChoicesHours = [0, 1, 6, 12, 24, 48, 168];

  static Future<int> getLampaSubUpdateHours() =>
      _lampaHours('lampa_sub_update_hours', 12);

  /// Null until the user saves the Lampa interval setting — AutoUpdater then
  /// keeps per-subscription `updateIntervalHours` (tests / classic UI).
  static Future<int?> peekLampaSubUpdateHours() async {
    final raw = await getVar('lampa_sub_update_hours', '');
    if (raw.isEmpty) return null;
    return int.tryParse(raw);
  }

  static Future<void> setLampaSubUpdateHours(int hours) =>
      setVar('lampa_sub_update_hours', hours.toString());

  static Future<int> getLampaAppCheckHours() =>
      _lampaHours('lampa_app_check_hours', 24);

  static Future<void> setLampaAppCheckHours(int hours) =>
      setVar('lampa_app_check_hours', hours.toString());

  static Future<int> getLampaGeoCheckHours() =>
      _lampaHours('lampa_geo_check_hours', 24);

  static Future<void> setLampaGeoCheckHours(int hours) =>
      setVar('lampa_geo_check_hours', hours.toString());

  static Future<DateTime?> getLampaLastAppCheck() async {
    final raw = await getVar('lampa_last_app_check_at', '');
    return raw.isEmpty ? null : DateTime.tryParse(raw);
  }
  ...
  /// Возвращает текущий tun-apps config. Default: `{mode: 'deny', packages: []}`
  /// (чёрный список / обход VPN, как в старой Lampa).
  ...
  static Future<Map<String, dynamic>> exportNativePrefsBackup() =>
      _exportToBackupMap();
  static Future<int> applyNativePrefsBackup(
  ...
  static Future<void> setWarpAccount(
    WarpAccount? account, {
    bool flush = true,
  ...
  static Future<void> setMasqueAccount(
