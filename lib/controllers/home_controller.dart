...
class HomeController extends ChangeNotifier
  ...
  HomeController({
    ...
    Future<void> Function(PriorityRoutingDecision decision)?
    onPriorityRoutingChanged,
  }) : _autoUpdater = autoUpdater,
       _ruleSetAutoUpdater = ruleSetAutoUpdater,
       _onPriorityRoutingChanged = onPriorityRoutingChanged;

  @override
  final BoxVpnClient _vpn = BoxVpnClient();
  final AutoUpdater? _autoUpdater;
  final Future<void> Function(PriorityRoutingDecision decision)?
  _onPriorityRoutingChanged;
  Timer? _priorityRoutingTimer;
  PriorityRoutingDecision? _priorityRoutingApplied;
  PriorityRoutingDecision? _priorityRoutingPending;
  static const priorityRoutingStability = Duration(seconds: 5);
  ...
  Future<void> refreshChannelLabels() async {
    ...
    _channelLabels = {for (final c in channels) c.tag: c.displayLabel};
  ...
  void _handleStatusEvent(TunnelStatusEvent event) {
    ...
    if (tunnel == TunnelStatus.connected) {
      ...
      unawaited(
        SettingsStorage.getCustomRules().then(
          (r) => RuleNameResolver.I.setRules(
            r,
            template: TemplateLoader.cachedOrNull(),
          ),
        ),
      );
      _startHeartbeat();
      ...
      _emit(
        _state.copyWith(
          ...
          lastStartError: reasonEn.isNotEmpty
              ? reasonEn
              : _state.lastStartError,
          lastStartErrorAt: reasonEn.isNotEmpty
              ? DateTime.now()
              : _state.lastStartErrorAt,
          ...
          configChangedNeedRestart: false,
          runningConfigRaw:
              _invalidateRunningConfig(), // §311 — running перестал существовать
        ),
      );
      ...
      if (tunnel == TunnelStatus.revoked) {
        ...
        AutomationEventEmitter.I.emitVpnError(
          'tunnel_error',
          event.errorReason!,
        );
    ...
    } else if (tunnel == TunnelStatus.stopping ||
        tunnel == TunnelStatus.connecting) {
      _stopHeartbeat();
  ...
  void _armTransientTimeout(TunnelStatus expected) {
    ...
    _transientTimeoutTimer = Timer(timeout, () async {
      ...
      _addDebug(
        DebugSource.app,
        'Timeout in ${expected.name}, forcing disconnect',
      );
      ...
      _addDebug(
        DebugSource.app,
        '[vpn] forceStopVPN sent (timeout in ${expected.name})',
      );
      if (_state.tunnel != expected) return;
      ...
      _emit(
        _state.copyWith(
          ...
          configChangedNeedRestart: false,
          runningConfigRaw:
              _invalidateRunningConfig(), // §311 — синтезированный down
        ),
      );
    });
  }
  ...
  /// §123 — foreground notification (no live speed — battery).
  ///   title = `Lampa · <human profile name>`
  ///   text  = route / group label
  Future<void> _pushNotificationLabels() async {
    const appName = 'Lampa';
    final remark =
        (await SettingsStorage.getVar('lampa_connection_remark', '')).trim();
    final profile = remark.isNotEmpty
        ? remark
        : _state.profileDisplayName(
            nodeLabelOf: (tag) {
    ...
    final title = profile.isEmpty || profile.toLowerCase() == 'vpn'
        ? appName
        : '$appName · $profile';

    // Subtitle: keep light — connection name already in title.
    final text = profile.isNotEmpty && profile.toLowerCase() != 'vpn'
        ? profile
        : '';
  ...
  Future<void> start() async {
    ...
    try {
      ...
      if (!ok) {
        _emit(
          _state.copyWith(lastError: const ErrMsg(ErrKey.failedToStartVpn)),
        );
      }
    } catch (e) {
  ...
  Future<void> _startCcStreams() async {
    ...
    _ccStatusSub = _cc.status.listen(
      _onCcStatus,
      onError: (Object e) {
        _addDebug(DebugSource.app, 'cc status stream error: $e');
      },
    );
    _ccGroupsSub = _cc.groups.listen(
      _onCcGroups,
      onError: (Object e) {
  ...
  static const _groupsPullMaxAttempts =
      12; // ~5с суммарно — щедрый STARTED- window
  ...
  void _startGroupsPull([int attempt = 0]) {
    ...
    _groupsPullTimer = Timer(_groupsPullStep, () async {
      ...
      if (groups.isEmpty) {
        ...
        _addDebug(
          DebugSource.app,
          '[cc] getGroups → empty (no selector groups)',
        );
        _applyGroups(groups);
        return;
      }
  ...
  void _applyGroups(List<CcGroup> ccGroups) {
    ...
    final routeFinal = RouteConfig.finalTag(next.activeConfigRaw);
    final routingRoot = groups.contains(routeFinal) ? routeFinal : initial;
    _observePriorityRouting(ccGroups, routingRoot);
  ...
  void _observePriorityRouting(List<CcGroup> groups, String? rootGroup) {
    final decision = priorityRoutingDecision(groups, rootGroup);
    if (decision == null || decision == _priorityRoutingApplied) {
      _priorityRoutingTimer?.cancel();
      _priorityRoutingPending = null;
      return;
    }
    if (decision == _priorityRoutingPending) return;
    _priorityRoutingPending = decision;
    _priorityRoutingTimer?.cancel();
    _priorityRoutingTimer = Timer(priorityRoutingStability, () async {
      if (_disposed || _priorityRoutingPending != decision) return;
      _priorityRoutingApplied = decision;
      _priorityRoutingPending = null;
      _addDebug(
        DebugSource.app,
        '[routing] stable ${decision.nodeTag}: '
        '${decision.p5Plus ? 'p5+' : 'default'} via ${decision.proxyOutbound}',
      );
      await _onPriorityRoutingChanged?.call(decision);
    });
  }
  ...
  void _recomputeDependencyHealth() {
    ...
    final hasDnsVictims = sick.values.any((list) => list.any((d) => d.isDns));
    if (banner != null) {
  ...
  Future<void> applyGroup(String? tag) async {
    ...
    _emit(
      _state.copyWith(nodes: nodes, activeInGroup: now, highlightedNode: now),
    );
  ...
  Future<void> switchNode(String nodeTag) async {
    ...
    try {
          ...
          await Future(() async {
            for (final id in ids) {
              try {
                ...
                /* соединение уже закрылось — игнор */
              }
            }
          }).timeout(const Duration(seconds: 5), onTimeout: () {});
          _addDebug(
            DebugSource.app,
            'Interrupted ${ids.length} conns in $group',
          );
        } catch (e) {
      ...
      AutomationEventEmitter.I.emitNodeChanged(
        prevNode,
        nodeTag,
        group,
        'user',
  ...
  Future<List<String>> _connectionIdsInGroup(String group) async {
    final conns = await _cc.connections.first.timeout(
