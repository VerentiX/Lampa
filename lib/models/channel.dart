...
int clampChannelTolerance(int v) =>
    v < 0 ? 0 : (v > _kToleranceMax ? _kToleranceMax : v);
...
enum UrltestMode {
  ...
  static UrltestMode fromWire(String? s) => UrltestMode.values.firstWhere(
    (m) => m.wire == s,
...
class ChannelAuto {
  ...
  ChannelAuto copyWith({
    ...
    tolerance: tolerance == null
        ? this.tolerance
        : clampChannelTolerance(tolerance),
    ...
    poolTolerance: poolTolerance == null
        ? this.poolTolerance
        : clampChannelTolerance(poolTolerance),
  ...
  factory ChannelAuto.fromJson(Map<String, dynamic> json) {
    ...
    final balMap = bal is Map<String, dynamic>
        ? bal
    ...
    return ChannelAuto(
      ...
      activeCheckInterval: json['active_check_interval'] as String? ?? '30s',
      activeCheckFailures:
          (json['active_check_failures'] as num?)?.toInt() ?? 2,
      tolerance: clampChannelTolerance(
        (json['tolerance'] as num?)?.toInt() ?? 50,
      ),
      ...
      poolTolerance: clampChannelTolerance(
        (balMap['pool_tolerance'] as num?)?.toInt() ?? 0,
      ),
